...
def sql_command2(cur, colnames, tablename, caseno_list):
    ...
    df = pd.DataFrame(cur.fetchall()).astype(str)
    df.columns = colnames
...
def function5():
    ...
    base64Image_list1, metadata_list1 = load_images(caseno_list1)  
    base64Image_list2, metadata_list2 = load_images(caseno_list2) 
    EndTime4 = time.time()
